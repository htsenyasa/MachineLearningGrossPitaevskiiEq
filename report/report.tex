...
With some manipulation in angular frequencies such as making $\omega_x, \omega_y \gg \omega_z$ and keeping their energy order much greater than condensate's energy $ \hbar(\omega_x \omega_y)^{1/2} \gg \mu $, dynamics of the system can be analyzed in one dimension with corresponding 1D GPE \textbf{REF}. To obtain 1D GPE one can rewrite wave function,
...
\textbf{determination of learning rate?}
...
In interacting systems values of interaction parameter $g$ are 0.1, 1, 10, 20. Here, we first give FNN results, after that CNN, and lastly separate energy predictions done by FNN. 
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
		...
		\label{fig:c}
    \end{subfigure}
	\caption{Notice that, the order of interaction energy is much smaller than the others. This may be the reason of bias in (a). }
\end{figure}
