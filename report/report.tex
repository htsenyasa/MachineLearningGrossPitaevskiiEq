...
\begin{subfigure}[t]{0.45\textwidth}
    ...
    \label{fig:d}
\end{subfigure}
\caption{Here the potential types given in labels. The interaction parameter $g$ of the random potentials is 15. The error between the numerical solution and approximation shrinks as the interaction parameter increases which corresponds to increment in interaction energy.}
\label{fig:comp_thomas_num}
\end{figure}

As shown in Fig~\ref{fig:comp_thomas_num}d, the interaction parameter is great enough to use Thomas-Fermi approximation but since the density is determined by the Eq~\eqref{eq:thomas_fermi_mu_integral} the characteristic must be same with the potential used. This situation can be seen vividly in Fig~\ref{fig:comp_thomas_num}c.
