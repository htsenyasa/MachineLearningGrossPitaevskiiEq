...
def display_plot2(info, file_name = None):
    ...
    for i in range(len(features)):
        ...
        relative_err.append(((info.testdataset[:,i] - info.predicted[:,i]) / (1e-15)) * 100)
        fig, ax1 = plt.subplots(num=features[i])
        left, bottom, width, height = [0.65, 0.20, .2, .2]
        if i != 0: inset = fig.add_axes([left, bottom, width, height])
        ...
        ax1.plot(info.testdataset[:,i], info.predicted[:,i], ".", markersize = marker_s)
