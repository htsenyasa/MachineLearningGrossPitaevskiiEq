...
def gather_wsdk_versions(conf, versions):
	...
	try:
		all_versions = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, 'SOFTWARE\\Wow6432node\\Microsoft\\Microsoft SDKs\\Windows')
	except WindowsError:
		try:
			all_versions = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, 'SOFTWARE\\Microsoft\\Microsoft SDKs\\Windows')
		except WindowsError:
			return
	index = 0
	while 1:
		...
		if not version_pattern.match(version):
			continue
		try:
			msvc_version = winreg.OpenKey(all_versions, version)
			path,type = winreg.QueryValueEx(msvc_version,'InstallationFolder')
		except WindowsError:
			continue
...
def gather_wince_supported_platforms():
	...
	try:
		ce_sdk = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, 'SOFTWARE\\Wow6432node\\Microsoft\\Windows CE Tools\\SDKs')
	except WindowsError:
		try:
			ce_sdk = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, 'SOFTWARE\\Microsoft\\Windows CE Tools\\SDKs')
		except WindowsError:
			ce_sdk = ''
	if not ce_sdk:
	...
	while 1:
		try:
			sdk_device = winreg.EnumKey(ce_sdk, ce_index)
		except WindowsError:
			break
		ce_index = ce_index + 1
		sdk = winreg.OpenKey(ce_sdk, sdk_device)
		try:
			path,type = winreg.QueryValueEx(sdk, 'SDKRootDir')
		except WindowsError:
			try:
				path,type = winreg.QueryValueEx(sdk,'SDKInformation')
...
def gather_icl_versions(conf, versions):
	...
	try:
		all_versions = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, 'SOFTWARE\\Wow6432node\\Intel\\Compilers\\C++')
	except WindowsError:
		try:
			all_versions = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, 'SOFTWARE\\Intel\\Compilers\\C++')
		except WindowsError:
			return
	index = 0
	while 1:
		...
		for target,arch in all_icl_platforms:
			try:
				...
				winreg.OpenKey(all_versions,version+'\\'+targetDir)
				icl_version=winreg.OpenKey(all_versions,version)
				path,type=winreg.QueryValueEx(icl_version,'ProductDir')
		...
		for target,arch in all_icl_platforms:
			try:
				icl_version = winreg.OpenKey(all_versions, version+'\\'+target)
				path,type = winreg.QueryValueEx(icl_version,'ProductDir')
				if os.path.isfile(os.path.join(path, 'bin', 'iclvars.bat')):
					try:
						targets.append((target, (arch, conf.get_msvc_version('intel', version, target, os.path.join(path, 'bin', 'iclvars.bat')))))
