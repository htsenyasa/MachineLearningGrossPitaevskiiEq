...
def main(argv=None):
  ...
  try:
    ...
    print("Exception raised while trying to read xmds script:", err, file=sys.stderr)
    return -1
  ...
  try:
    ...
    print("XML Parser error:", err, file=sys.stderr)
    if debug: raise
    return -1
  except Exception as err:
    print("Exception raised during parsing xmds script:", err, file=sys.stderr)
    if debug: raise
    return -1
  ...
  if os.path.isfile(xpdeintDataCachePath):
    try:
      ...
      print("Warning: Unable to load xmds2 data cache.", file=sys.stderr)
      if debug: raise
  ...
  try:
    ...
    if not parser:
      print("Unable to recognise file as an xmds script of the correct version.", file=sys.stderr)
      return -1
  ...
  # error string that should be presented to the user.
    ...
    if not lineNumber == None:
      ...
      if not columnNumber == None:
          positionReference.append(" "*(columnNumber-1) + "^~~ here.")
      print(''.join(positionReference), file=sys.stderr)
    if debug:
      if err.element:
        print("    In element: " + err.element.userUnderstandableXPath(), file=sys.stderr)
      else:
  ...
  if dataCache:
    try:
      ...
      print("Warning: Unable to write xmds2 data cache. " \
                           "Ensure '%(xpdeintUserDataPath)s' exists and is writable." % locals(), file=sys.stderr)
    else:
