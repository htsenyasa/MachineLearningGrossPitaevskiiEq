...
def main(argv=None):
  ...
  try:
    ...
    if userSpecifiedFilename and len(args) > 1:
      print("The '-o' option cannot be used when processing multiple xsil files.", file=sys.stderr)
    if not args:
  ...
  for xsilInputName in args:
    ...
    try:
      ...
      print("Exception raised while trying to read xsil file:", err, file=sys.stderr)
      if debug:
        raise
      return
