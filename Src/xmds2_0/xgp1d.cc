...
int main(int argc, char **argv)
{
  ...
  while (1) {
    ...
    resp = getopt_long(argc, argv, "hi:n:p:r:a:", long_options, &option_index);
...
void _print_usage()
{
  ...
  _LOG(_NO_ERROR_TERMINATE_LOG_LEVEL, "\n\nUsage: xgp1d --interaction_param <real> --num_particles <real> --pot_type <real> --runtime_id <integer> --alpha <real>\n\n"
                         "Details:\n"
...
void _write_xsil_header(FILE* fp)
{
  ...
  fprintf(fp, "      ]]>\n");
  fprintf(fp, "      <argument default_value=\"0.5\" name=\"alpha\" type=\"real\"/>\n");
  fprintf(fp, "    </arguments>\n");
  fprintf(fp, " </features>\n");
  fprintf(fp, "\n");
  fprintf(fp, "\n");
  ...
  fprintf(fp, "        Ncalc = mod2(phi);\n");
  fprintf(fp, "        EN = alpha * mod2(dphix) + (V1 + 0.5 * Uint * mod2(phi)) * mod2(phi);\n");
  fprintf(fp, "        Ekin = alpha * mod2(dphix);\n");
  fprintf(fp, "        Epot = V1 * mod2(phi);\n");
  fprintf(fp, "        Eint = 0.5 * Uint * mod2(phi) * mod2(phi);\n");
  fprintf(fp, "                Virial = Ekin - Epot + Eint;\n");
  fprintf(fp, "                mu = Ekin + Epot + 2 * Eint;\n");
  ...
  fprintf(fp, "  Command line argument alpha = %e\n", alpha);
  fprintf(fp, "</info>\n");
