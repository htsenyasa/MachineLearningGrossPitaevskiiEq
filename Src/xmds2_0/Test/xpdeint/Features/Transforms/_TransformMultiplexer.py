...
class _TransformMultiplexer (_Feature):
  ...
  def buildTransformMap(self):
    ...
    def printBasis(basis):
      if isinstance(basis, str): return basis
      elif len(basis) == 1:
        return basis[0] if isinstance(basis[0], str) else basis[0].canonicalName
      else:
        return '(' + ', '.join(dimRep if isinstance(dimRep, str) else dimRep.canonicalName for dimRep in basis) + ')'
    ...
    for vector in vectors:
      ...
      for basisPair in combinations(2, vectorBases):
        ...
        for (currentBasis, basisState), (transformID, transformPair) in zip(path[:-2:2], path[1::2]):
          ...
          runtimePrefixDimensions = tuple([dimRep.localLattice for dimRep in prefixDimReps if dimRep.hasLocalOffset or isinstance(dimRep.runtimeLattice, str)])
          runtimePostfixDimensions = tuple([dimRep.localLattice for dimRep in postfixDimReps if dimRep.hasLocalOffset or isinstance(dimRep.runtimeLattice, str)])
          
          prefixLattice = [(dimRep.latticeEstimate, dimRep.localLattice) for dimRep in prefixDimReps if not (dimRep.hasLocalOffset or isinstance(dimRep.runtimeLattice, str))]
          
          postfixLattice = [(dimRep.latticeEstimate, dimRep.localLattice) for dimRep in postfixDimReps if not (dimRep.hasLocalOffset or isinstance(dimRep.runtimeLattice, str))]
