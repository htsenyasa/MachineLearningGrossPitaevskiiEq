...
// vector raw defines
#define _mg1_output_raw_ncomponents 8
...
int main(int argc, char **argv)
{
  ...
  while (1) {
    static struct option long_options[] = 
      {
        ...
        {"runtime_id", required_argument, 0, 'r'},
    ...
    resp = getopt_long(argc, argv, "hi:n:p:r:A:B:L:", long_options, &option_index);
    ...
    switch (resp) {
      ...
      case 'A':
        Alpha = strtol(optarg, NULL, 10);
        break;
      
      case 'B':
        Beta = strtol(optarg, NULL, 10);
        break;
      
      case 'L':
        LL = strtol(optarg, NULL, 10);
        break;
...
void _print_usage()
{
  ...
  _LOG(_NO_ERROR_TERMINATE_LOG_LEVEL, "\n\nUsage: xgp1d --interaction_param <real> --num_particles <real> --pot_type <real> --runtime_id <integer> --Alpha <integer> --Beta <integer> --LL <integer>\n\n"
                         "Details:\n"
                         "Option\t\tType\t\tDefault value\n"
                         "-i,  --interaction_param\treal \t\t0\n"
                         "-n,  --num_particles\treal \t\t1\n"
                         "-p,  --pot_type\treal \t\t0\n"
                         "-r,  --runtime_id\tinteger \t\t5\n"
                         "-A,  --Alpha\tinteger \t\t0.5\n"
                         "-B,  --Beta\tinteger \t\t0\n"
                         "-L,  --LL\tinteger \t\t0\n"
                         );
...
real _segment3_setup_sampling(bool* _next_sample_flag, long* _next_sample_counter)
{
  ...
  if (_next_sample_counter[0] * _previous_M == _previous_m * 10) {
    ...
    _t_break_next = _next_sample_counter[0] * ((real)20.0) / ((real)10);
...
void _write_xsil_header(FILE* fp)
{
  ...
  fprintf(fp, "        real alpha, beta, gamma_, L; // scale coeffs\n");
  ...
  fprintf(fp, "      ]]>\n");
  fprintf(fp, "\n");
  fprintf(fp, "      <argument default_value=\"0.5\" name=\"Alpha\" type=\"integer\"/>\n");
  fprintf(fp, "      <![CDATA[\n");
  fprintf(fp, "      alpha = Alpha;\n");
  fprintf(fp, "      ]]>\n");
  fprintf(fp, "      <argument default_value=\"0\" name=\"Beta\" type=\"integer\"/>\n");
  fprintf(fp, "      <![CDATA[\n");
  fprintf(fp, "      beta = Beta;\n");
  fprintf(fp, "      ]]>\n");
  fprintf(fp, "      <argument default_value=\"0\" name=\"LL\" type=\"integer\"/>\n");
  fprintf(fp, "      <![CDATA[\n");
  fprintf(fp, "      L = LL;\n");
  fprintf(fp, "      ]]>\n");
  fprintf(fp, "\n");
  ...
  fprintf(fp, "      <samples>10 1 1</samples>\n");
  ...
  fprintf(fp, "        <moments>norm e1 e1kin e1pot e1int vir1 mu1 gg</moments>\n");
  ...
  fprintf(fp, "  Command line argument Alpha = %li\n", Alpha);
  
  fprintf(fp, "  Command line argument Beta = %li\n", Beta);
  
  fprintf(fp, "  Command line argument LL = %li\n", LL);
  fprintf(fp, "</info>\n");
...
void _mg1_sample()
{
  ...
  #define gg _active_mg1_output_raw[_mg1_output_raw_index_pointer + 7]
  // Set index pointers explicitly for (some) vectors
...
void _mg1_write_out(FILE* _outfile)
{
  ...
  if (_outfile) {
    ...
    fprintf(_outfile, "  <Array Name=\"variables\" Type=\"Text\">\n");
    fprintf(_outfile, "    <Dim>9</Dim>\n");
    fprintf(_outfile, "    <Stream><Metalink Format=\"Text\" Delimiter=\" \\n\"/>\n");
    fprintf(_outfile, "t norm e1 e1kin e1pot e1int vir1 mu1 gg \n");
    fprintf(_outfile, "    </Stream>\n");
    fprintf(_outfile, "  </Array>\n");
    fprintf(_outfile, "  <Array Name=\"data\" Type=\"double\">\n");
  ...
  #endif
  hid_t dataset_gg;
  if (!H5Lexists(hdf5_file, "/2/gg", H5P_DEFAULT))
    dataset_gg = H5Dcreate(hdf5_file, "/2/gg", H5T_NATIVE_REAL, file_dataspace, H5P_DEFAULT);
  else
    dataset_gg = H5Dopen(hdf5_file, "/2/gg");
  #if defined(HAVE_HDF5_HL)
    H5DSattach_scale(dataset_gg, dataset_t, 0);
  #endif
  H5Dclose(dataset_t);
  ...
  if ((_mg1_output_lattice_t)) {
    ...
    if (dataset_gg)
      H5Dwrite(dataset_gg, H5T_NATIVE_REAL, mem_dataspace, file_dataspace, H5P_DEFAULT, _active_mg1_output_raw);
