...
class _DeltaAOperator (Operator):
  ...
  def preflight(self):
    ...
    for componentName, resultDict, codeSlice in indexAccessedVariables:
      ...
      dimRepNamesForThisVectorNeedingReordering = [dimRepName for dimRepName, (indexString, accessLoc) in resultDict.items() if indexString != dimRepName]
    ...
    indexAccessedDerivatives = CodeParser.nonlocalDimensionAccessForComponents(list(derivativeMap.keys()), self.primaryCodeBlock)
    
    for componentName, resultDict, codeSlice in reversed(indexAccessedDerivatives):
      componentAccessString = componentName
      componentAccesses = []
      for dimRepName, (accessString, accessCodeLoc) in resultDict.items():
